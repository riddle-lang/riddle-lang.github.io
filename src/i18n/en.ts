...
export const en: SiteContent = {
  ...
  hero: {
    badge: 'v0.2.3 · Technology preview',
    title: 'Memory safety,',
...
void rgc_collect(void);`,
  ...
  release: {
    eyebrow: 'What\'s new in v0.2.3',
    title: 'A more expressive language and a broader library',
    subtitle:
    ...
    items: [
      {
        title: 'Bracket lambdas',
        desc: '`[v -> v * 2]` writes a short anonymous function with inferred parameter and return types, and `values.map [v -> v * 2]` calls it inline. They lower to the same closures as `fun`; `vec![a, b]` and `0..5` ranges round out the new syntax.',
      },
      {
        title: 'A broader standard library',
        desc: 'Collections gained `remove`, `insert` and `sort`; iterators gained lazy `map` / `filter` / `zip` and eager `count` / `fold` / `find`; new `std::fs`, `std::time`, `std::random` and `std::parse` modules cover files, sleeps, randomness and number parsing.',
      },
      {
        title: 'A sharper language server',
        desc: 'Nine diagnostic-derived quick fixes, lambda-parameter and method-chain type hints, `Clue.toml` schema diagnostics with completions, organize imports, expand selection, document links and pull diagnostics.',
      },
      {
        title: 'Compiler foundations',
        desc: '`riddlec` accepts multiple input files, constants evaluate at compile time for array lengths and const generics, slices cross `unsafe extern "C"` as pointer/length pairs, and the GC uses a hashed object registry with an adaptive threshold.',
      },
    ],
  },
  toolchain: {
    ...
    items: [
      ...
      },
      {
        ...
        desc: 'Creates, checks, builds and runs Riddle projects. Binary projects produce a native executable; library projects emit `.rlib` / `.rmeta` metadata plus static or dynamic libraries.',
