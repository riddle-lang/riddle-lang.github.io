...
export const en: SiteContent = {
  ...
  hero: {
    badge: 'v0.2.2 · Technology preview',
    title: 'Memory safety,',
...
void rgc_collect(void);`,
  ...
  release: {
    eyebrow: 'What\'s new in v0.2.2',
    title: 'A fuller toolchain and editor experience',
    subtitle:
      'This release adds a standalone `riddle fmt` command and fixes generic enum inference, dynamic trait dispatch and completion edit ranges.',
    items: [
      {
        title: 'A shared source formatter',
        desc: '`riddle fmt` formats files or standard input, supports `--check`, indentation width and hard tabs; LSP formatting requests reuse the same implementation while preserving comments, attributes and line endings.',
      },
      {
        title: 'Dynamic traits and inference',
        desc: 'Dynamic-trait object-safety checks, associated-type substitution and explicit generic arguments on enum variants are now more reliable instead of silently accepting invalid states.',
      },
      {
        title: 'More precise LSP completion',
        desc: 'Completion items now return exact text-edit ranges and plain-text insertion, while workspace indexing and automatic imports continue to cover cross-file editing.',
      },
      {
        title: 'Four distribution commands',
        desc: 'Prebuilt toolchains and source installs now include `clue`, `riddlec`, `riddle` and `riddle-lsp`; `riddle` provides the unified CLI entry point.',
      },
    ],
  },
  toolchain: {
    eyebrow: 'Toolchain',
    title: 'One manager, four binaries',
    subtitle:
    ...
    items: [
      ...
      {
        name: 'riddle',
        tagline: 'Unified tools CLI',
        desc: 'Provides the unified Riddle tools entry point, currently with source formatting.',
        usage: 'riddle fmt [--emit files|stdout|check] [--check] <file>...',
        points: [
      ...
      },
      {
        ...
        points: [
          ...
          'Workspace projects generate a lockfile to pin resolved dependency versions',
  ...
  editors: {
    ...
    missTitle: 'Still evolving',
    miss: ['More extensive semantic refactoring actions'],
  },
  quickstart: {
    eyebrow: 'Get started',
...
clue run`,
  ...
  status: {
    ...
    subtitle:
      'The v0.2.2 language and toolchain may still change incompatibly. Here is the honest boundary of what works.',
    ...
    works: [
      ...
      'Formatted output and standard panic / assert / todo macros',
    ...
    limits: [
      '`format!` and related formatting macros do not yet support indexed, named or other format specifiers',
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          'Dependency management in `clue`: a registry, version resolution and git dependencies',
