...
void rgc_collect(void);`,
  ...
  release: {
    eyebrow: 'v0.2.2 新内容',
    title: '更完整的工具链与编辑器体验',
    subtitle: '这一版加入独立的 `riddle fmt`，并修复泛型枚举推断、动态 trait 分发和补全编辑范围。',
    items: [
      {
        title: '共享源码格式化器',
        desc: '`riddle fmt` 支持文件、标准输入、`--check`、缩进宽度和硬制表符；LSP 格式化请求复用同一实现，并保留注释、属性和换行风格。',
      },
      {
        title: '动态 trait 与类型推断',
        desc: '动态 trait 对象安全检查、关联类型替换和显式泛型枚举变体参数更加可靠，错误状态不会静默通过。',
      },
      {
        title: '更精确的 LSP 补全',
        desc: '补全返回明确的 text edit 范围和纯文本插入格式，工作区索引与自动导入继续覆盖跨文件场景。',
      },
      {
        title: '四个发行命令',
        desc: '预编译工具链与源码安装现在包含 `clue`、`riddlec`、`riddle` 和 `riddle-lsp`；`riddle` 提供统一 CLI 入口。',
      },
    ],
  },
  toolchain: {
...
clue run`,
  ...
  status: {
    eyebrow: '项目状态',
    title: '这是技术预览，我们不藏着',
    subtitle: 'v0.2.2 的语言和工具链仍可能发生不兼容变化。下面是当前真实的能力边界。',
    worksTitle: '已经可用',
    works: [
    ...
    limitsTitle: '当前限制',
    limits: [
      '`format!` 等格式宏暂不支持索引参数、命名参数和其他格式说明符',
    ...
    phases: [
      ...
      },
      {
        ...
        items: [
          ...
          '`clue` 依赖管理:registry、版本解析与 git 依赖',
