...
void rgc_collect(void);`,
  ...
  release: {
    eyebrow: 'v0.2.3 新内容',
    title: '更有表达力的语言与更宽的标准库',
    subtitle: '这一版加入括号 lambda、`vec!` 与 range 表达式、迭代器适配器、新的文件 / 时间 / 随机数 / 解析模块，LSP 也带来九个快速修复与 `Clue.toml` 支持。',
    items: [
      {
        title: '括号 lambda',
        desc: '`[v -> v * 2]` 用短语法写匿名函数，参数与返回类型自动推断，`values.map [v -> v * 2]` 可以就地调用；它与 `fun` 闭包共享同一底层表示，`vec![a, b]` 和 `0..5` range 一并到位。',
      },
      {
        title: '更宽的标准库',
        desc: '集合获得 `remove`、`insert` 和 `sort`；迭代器新增惰性 `map` / `filter` / `zip` 与即时 `count` / `fold` / `find`；新的 `std::fs`、`std::time`、`std::random`、`std::parse` 模块覆盖文件、休眠、随机数与数字解析。',
      },
      {
        title: '更敏锐的语言服务器',
        desc: '九个由诊断驱动的快速修复、lambda 参数与方法链类型提示、`Clue.toml` schema 诊断与补全、整理导入、扩展选区、文档链接与拉取式诊断。',
      },
      {
        title: '编译器地基',
        desc: '`riddlec` 支持多输入文件，常量在编译期求值用于数组长度与 const generics，切片以指针/长度对跨越 `unsafe extern "C"`，GC 改用哈希对象注册表并自适应调整回收阈值。',
      },
    ],
  },
  toolchain: {
    ...
    manager: {
      ...
      points: [
        ...
        '把 ridup 复制或硬链接成 `clue`、`riddlec`、`riddle`、`riddle-lsp` 后，它会代理到选中的工具链',
    ...
    items: [
      ...
      },
      {
        name: 'riddle',
        tagline: '统一工具 CLI',
        desc: '提供 Riddle 工具链的统一入口，当前包含源码格式化命令。',
      ...
      },
      {
        name: 'clue',
        tagline: '项目构建器',
        desc: '创建、检查、构建和运行 Riddle 项目。二进制项目会生成本机可执行文件，库项目可输出 `.rlib` / `.rmeta` 元数据以及静态库或动态库。',
...
clue run`,
  ...
  status: {
    eyebrow: '项目状态',
    title: '这是技术预览，我们不藏着',
    subtitle: 'v0.2.3 的语言和工具链仍可能发生不兼容变化。下面是当前真实的能力边界。',
    worksTitle: '已经可用',
    works: [
    ...
    limits: [
      ...
      '闭包按整个绑定捕获，不做字段级精确捕获',
      '没有裸函数指针类型；可调用值需用 `impl Fn(...) -> T` 或 `dyn Fn(...)` 声明',
    ],
